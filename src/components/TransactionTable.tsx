      ...
      <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 6, overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 1160 }}>
            <thead>
              <tr>
                {['EFRM Status', 'TXN ID', 'Timestamp', 'Amount', 'Channel', 'Type', 'Payee', 'New Payee', 'Category', 'City', 'Alerts'].map(h => (
            ...
            <tbody>
              {slice.map((tx, i) => {
                ...
                return (
                  ...
                  >
                    {/* EFRM Status */}
                    <td style={{ ...TD_BASE, width: 80, textAlign: 'center' }}>
                      {isFlagged ? (
                        <span title={`${txAlerts.length} rule${txAlerts.length > 1 ? 's' : ''} triggered`} style={{ fontSize: 14 }}>🚨</span>
                      ) : (
              ...
              {slice.length === 0 && (
                <tr><td colSpan={11} style={{ padding: '28px', textAlign: 'center', color: COLORS.muted, fontSize: 13 }}>No transactions match the current filters.</td></tr>
              )}
            </tbody>
          </table>
        </div>
